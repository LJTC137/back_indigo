...
export class EquipoServicioEntity{
    ...
    //============== Foreign keys
    //======= Alquiler Equipo
    @OneToMany(() => AlquilerXEquipoEntity, (alquilerEquipo) => alquilerEquipo.equipo)
    equipoAlquiler: AlquilerXEquipoEntity[];

    //======= Catalogo
    @ManyToOne(() => CatalogoEntity, (catalogo) => catalogo.catalogoTipoContratacion, { nullable: false })
    @JoinColumn({ name: 'tipoContratacion' })
    tipoContratacion: CatalogoEntity;

    @ManyToOne(() => CatalogoEntity, (catalogo) => catalogo.catalogoTipoEquipo, { nullable: false })
